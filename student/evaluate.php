...
$x= $_SESSION['username'];
$aid=$_SESSION['aid'];
$sql="SELECT `file`, `sub_id` FROM `student_upload` WHERE `id`='$aid' "; 
	...
	$number = count($_POST['marks']);    
      ...
      {  
           ...
           {  
                ...
                $sql = "INSERT INTO `student_evaluate`(`username`, `sub_id`, `file`, `max_marks`, `marks`, `remarks`) VALUES ('$x','$sub_id','$file', '$max_marks','$marks','$remarks')"; 
                $run=mysqli_query($con, $sql);  
           }  
      }
	$query1="SELECT `paper_evaluated` FROM `student` WHERE `username`='$x'";
	$run_query1=mysqli_query($con,$query1);
	$data_q=mysqli_fetch_assoc($run_query1);
	$y=$data_q['paper_evaluated'];
	$y=$y+1;
	$query2="UPDATE `student` SET `paper_evaluated`='$y' WHERE `username`='$x'";
