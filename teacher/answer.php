...
$sql="SELECT `file`, `student_name`, `branch`, teacher_upload.`sub_id`, teacher_upload.`id` FROM `teacher_upload` LEFT JOIN `subject` ON teacher_upload.`sub_id`=subject.`sub_id` WHERE subject.`user_type`='Teacher' AND subject.`username`='$username' GROUP BY(`file`)"; 
